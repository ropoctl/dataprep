...
def clean_isin(
    ...
    """
    Clean ISIN type data in a DataFrame column.
    ...
    >>> clean_isin(df, 'isin', inplace=True)
           isin_clean
    0    US0378331005
    1    GB00BYXJL758
    """
    ...
    if split:
        ...
        df = df.assign(
            ...
            national_security_identifier=df["clean_code_tup"].map(
                itemgetter(2), meta=("national_security_identifier", object)
            ),
            check_digit=df["clean_code_tup"].map(itemgetter(3), meta=("check_digit", object)),
        )
    else:
        df = df.assign(
...
def validate_isin(
    ...
    """
    Validate if a data cell is ISIN in a DataFrame column. For each cell, return True or False.
