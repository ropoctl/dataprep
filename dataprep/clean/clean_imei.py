...
def clean_imei(
    ...
    """
    Clean IMEI type data in a DataFrame column.
    ...
    >>> clean_imei(df, 'imei', inplace=True)
           imei_clean
    0  35-686800-004141-20
    1  35-417803-685978-9
    """
    ...
    if split:
        ...
        df = df.assign(
            ...
            tac_code=df["clean_code_tup"].map(itemgetter(1), meta=("country_code", object)),
            serial_number=df["clean_code_tup"].map(itemgetter(2), meta=("country_code", object)),
...
def validate_imei(
    ...
    """
    Validate if a data cell is IMEI in a DataFrame column. For each cell, return True or False.
...
def _format(
    ...
    if split:
        ...
        if len(compacted_val) == 14:
            result = [compacted_val[:8], compacted_val[8:14], np.nan]
        else:
            result = [compacted_val[:8], compacted_val[8:14], compacted_val[14:]]
    if output_format == "compact":
        result = [imei.compact(val)] + result
    elif output_format == "standard":
        result = [imei.format(val)] + result
